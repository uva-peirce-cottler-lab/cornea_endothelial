...
for n = 1:numel(xcords)
      add_bar(xcords(n),ydatas(:,n),bar_width)
end

% Draw vertical bars
plot(xcords, mean(ydatas,1),'k','Linewidth',1)
% hold off
end


function add_bar(xcord,ydata,bar_width)

bar_y_min = mean(ydata(:,1))-std(ydata(:,1))/2;
bar_y_max = mean(ydata(:,1))+std(ydata(:,1))/2;

% Plot vertical for each bar
plot([xcord xcord], [bar_y_min bar_y_max],'k','Linewidth',1);



% Plot top and bottom 
plot([xcord(1)-bar_width/2 xcord(1)+bar_width/2],[bar_y_max bar_y_max],'k','Linewidth',1)
plot([xcord(1)-bar_width/2 xcord(1)+bar_width/2],[bar_y_min bar_y_min],'k','Linewidth',1)

%Plot Mid
plot([xcord(1)-bar_width/1.2 xcord(1)+bar_width/1.2],[mean(ydata(:,1)) ...
    mean(ydata(:,1))],'k','Linewidth',1.25)
